\section{The model}
  ...
  \ExplainBox{Why all the formulas contains an expected value on \(\prob{\vec{v}}\)?}{
    ...
    Since we have to figure out a way to sample complex distribution, it's better having
    only one instead of one for each node. In example, for the derivative respect to \(c_j\)
    I have to take the expected value on the distribution \(\prob{h_j}\), but using the fact
    ...
    an easy way\footnote{Well, it's not fortune... Classic binary RBM are so important
    because of this little trick that makes training far more easier than others BMs
    \& RBMs.}.
  }