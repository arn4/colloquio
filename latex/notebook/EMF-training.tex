...
\PhysicsBox{What are \(F\) and \(\vec{q}\) in  physics?}{
  ...
  like a local parameter, but since the are a microscopic property they haven't a 
  macroscopic thermodynamic quantity associated. Instead since \(\vec{q}\) is controlled
  from the extern of the system can be treated as thermodynamic variable.
...
\ExplainBox{The limit \(\beta \to 0\) for \(\vec{\lambda}\)}{
 ...
 \PhysicsBox{Why this makes sense physically}{
   ...
   correspond to high temperatures, so the system it's much more disordered by thermal
   fluctuations. If we want to keep the magnetization fixed (so impose some kind of order)
   we have to use a  strong field in order to overcome the effect of temperature.
...
\MyIdeaBox{Solving these equations}{
  ...
  is just start from a random guess of \(\vec{m}\), and then iterate until it converges to a
  desired precision.
  
  I'm quite convinced that this is far from optimal solution for this. Maybe a smarter
  implementation of finding solutions can speed up the algorithm noticeably.
}
