...
\begin{frame}
  ...
  \]
  \begin{itemize}
    \item \structure{order 0} it's the entropy, as predicted by thermodynamic
    \[
      A{\left[\vec{m};0\right]} = S{\left(\vec{m}\right)} \coloneqq -\sumt\left[\log{\left(m_t\right)}m_t + \log{\left(1-m_t\right)}(1-m_t)\right]
     \]
    \item \structure{order 1} energy of magnetizations
    \[
